...
def oov_trans_failed(predictions,
                 labels,
                 weights_fn=common_layers.weights_nonzero):
  """Percentage of particular vocab that translated correctly."""
  ...
  for word in dict2num:
    label_word = tf.ones_like(padded_labels) * word  # batch*len*1*1
    label_mask = tf.to_int32(tf.equal(padded_labels, label_word))
    outputs = tf.to_int32(tf.argmax(padded_predictions, axis=-1))
    axis = list(range(1, len(outputs.get_shape())))
  ...
  return tf.to_float(tf.div(tf.add_n(scores),tf.add_n(total))), tf.constant(1.0)
...
def create_evaluation_metrics(problems):
  ...
  metrics_list = [("oov_trans_failed",oov_trans_failed)]
