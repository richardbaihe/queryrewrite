...
#from tensor2tensor.models import models
from tensor2tensor.data_generators import problem_hparams
from tensor2tensor.utils import estimators
...
flags.DEFINE_integer("local_eval_frequency", 500,
...
flags.DEFINE_bool("decode_return_beams", False,
                  ...
                  "<beam1>\t<beam2>..\t<input>")
flags.DEFINE_integer("save_checkpoints_steps",40,"save_checkpoints_steps")
...
def create_experiment(output_dir, data_dir, model_name, train_steps,
  ...
  for hparams_set in hparams_set_list:
      hparams = create_hparams(hparams_set, data_dir)
      hparams_list.append(hparams)
  print('hparams_list',len(hparams_list))
...
def model_builder(model, hparams):
  ...
  def model_fn(features, targets, mode,reward=None,baseline=None):
    ...
    def nth_model(n):
      ...
      # model_class = registry.model(model)(   ##t2t_model.py ?
      model_class = transformer.Transformer(   ##t2t_model.py ?
          hparams, mode, hparams.problems[n],
          n, dp, _ps_devices(all_workers=True), hparams_list=hparams_list)
      ...
      if FLAGS.self_critical:
          #random_sample = tf.random_uniform(tf.shape(global_step),minval=0,maxval=10,dtype=tf.int32)
          #step_sc = tf.constant(800, dtype=tf.float32)
          #random_sc = tf.constant(5, dtype=tf.int32)
          #sharded_logits, training_loss, extra_loss = tf.cond(global_step>step_sc ,
          #                                                    true_fn= lambda : tf.cond(random_sample<random_sc,
          #                                                                              lambda : model_class.self_critic(  ##!
          #    features,skip=(skipping_is_on and skip_this_one)), lambda : model_class.model_fn( ##!
          #    features, skip=(skipping_is_on and skip_this_one))),
          #                                                    false_fn= lambda : model_class.model_fn( ##!
          #    features, skip=(skipping_is_on and skip_this_one)))
          sharded_logits, training_loss, extra_loss =model_class.self_critic(features,skip=(skipping_is_on and skip_this_one))
      else:
    ...
    if FLAGS.self_critical:
        return global_step,total_loss, train_op
    else:
...
def run_locally(exp):
  ...
  if FLAGS.decode_from_file is not None:
...
def decode_from_file(estimator, filename):
  ...
  #  for input_gen_one in input_gen:
  #      gen_fn = make_input_fn_from_generator(input_gen_one)
  #      example = gen_fn()
  #      yield _decode_input_tensor_to_features_dict(example, hparams)
  ...
  decodes = []
  scores = []
  estimator.get_session()
  # result_iter = estimator.predict(input_fn=input_fn, as_iterable=True)
  # print("result_iter:",result_iter,type(result_iter))
  for input_x in input_fn:
    #input_x = input_fn.next
    result_iter = estimator.predict(input_fn=input_x, feed_fn=input_x)
  #for _ in range(num_decode_batches):
    ...
    for result in result_iter:
      ...
      if FLAGS.decode_return_beams:
        ...
        tf.logging.info("Inference results SCORES: %s" % ', '.join(result["scores"]))
        scores.append("\n".join(result["scores"]))
      else:
  ...
  if FLAGS.decode_to_file:
    decode_filename = FLAGS.decode_to_file
    decode_filename_score = FLAGS.decode_to_file + ".score" ##!!
  else:
...
def _decode_input_tensor_to_features_dict(feature_map, hparams):
  ...
  """
        ...
        #feature_map = feature_map.next()
  inputs = tf.convert_to_tensor(feature_map["inputs"])
  #else:
  #      inputs = tf.convert_to_tensor(feature_map.next()["inputs"])
  ...
  def input_fn():
    ...
    with tf.name_scope("input_queues"):
      for n in xrange(problem_count):
        ...
        while len(feature_map["sc_inputs"].get_shape()) != 4:
          feature_map["sc_inputs"] = tf.expand_dims(feature_map["sc_inputs"], axis=-1)
        while len(feature_map["targets"].get_shape()) != 4:
          feature_map["targets"] = tf.expand_dims(
              feature_map["targets"], axis=-1)

        batches.append(
            (feature_map["sc_inputs"],feature_map["inputs"], feature_map["targets"], tf.constant(n),
    ...
    if fixed_problem is None:
      ...
      rand_sc_inputs,rand_inputs, rand_target, choice, inp_id, tgt_id = _cond_on_index(
      ...
      rand_sc_inputs,rand_inputs, rand_target, choice, inp_id, tgt_id = batches[0]

    # Set shapes so the ranks are clear.
    rand_sc_inputs.set_shape([None, None, None, None])
...
def model_builder_visual(model, hl):
  """Returns a function to build the model.
  ...
  def model_fn(features, targets, mode,hparams_list=hl,reward=None,baseline=None):
    """Creates the prediction, loss, and train ops.
    ...
    def nth_model(n):
      ...
      if FLAGS.self_critical:
          if reward==None:
              model_class._hparams.sampling_method = "random"
              sample = model_class._greedy_infer(features, 50, False)
              model_class._hparams.sampling_method = "argmax"
              greedy = model_class._greedy_infer(features, 50, False)
              r = model_class.slot_miss_percent(sample,features['targets'])
              b = model_class.slot_miss_percent(greedy, features['targets'])
              return sample, greedy ,r,b
          else:
              sharded_logits, training_loss, extra_loss = model_class.self_critic(features,reward,baseline,skip=(skipping_is_on and skip_this_one))
      else:
          sharded_logits, training_loss, extra_loss = model_class.model_fn( ##!
              features, skip=(skipping_is_on and skip_this_one))
      with tf.variable_scope("losses_avg", reuse=True):
