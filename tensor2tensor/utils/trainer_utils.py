...
from utils import my_beam_search_test
...
# def decode_from_file(hparams, filename):
  ...
  decodes = []
  beamsearch = my_beam_search_test.BeamSearchTest(hparams, hparams.problems[problem_id],4)
  # beamsearch = test.TransformerTest(hparams, hparams.problems[problem_id])
  ...
  for features in input_fn:
    ...
    #   as_iterable=True)
    print('inputs batch size: ', features['inputs'].shape)
    result_iter = beamsearch.testShapes(np.expand_dims(features["inputs"],axis=-1))
    print('batch: %d' % i)
    i += 1
   # result_iter = beamsearch.testTransformer(features["inputs"])
    j = 1
   ...
   # for _ in range(1):
      ...
      if FLAGS.decode_return_beams:
        ...
        # for k in range(len(result["outputs"])):
