...
def _make_metrics_ops(metrics, features, labels, predictions):
  ...
  for name, metric in sorted(six.iteritems(metrics)):
    if isinstance(metric, metric_spec.MetricSpec):
      # tf.logging.info('name of metrics:%s', type(name))
      # tf.logging.info('name of metrics:%s',name)
      #TODO: add oov_trans_failed in summary
      # if name=="metrics-wmt_ende_bpe32k/oov_trans_failed" or name=='metrics/oov_trans_failed':
...
class BaseEstimator(
  ...
  def get_monitored_session(self, input_fn):
    # all_hooks = []
  ...
  def get_session(self, outputs=None): #, input_fn,outputs=None):
    ...
    with ops.Graph().as_default() as g:
      ...
      self.inputs_ph = tf.placeholder(tf.int32,[None,None])
      #self.problem_ph = tf.placeholder(tf.int32,[1])
      features = {"inputs":self.inputs_ph,'problem_choice':np.array(0)}
      #print(features)
      #self.features = self._get_features_from_input_fn(input_fn)
      #print('input :')
      #print(self.features['inputs'].shape)
      #print('problem_choice: ')
      #print(self.features['problem_choice'].shape)
      infer_ops = self._get_predict_ops(features)
      self.predictions = self._filter_predictions(infer_ops.predictions, outputs)
      self.mon_sess = monitored_session.MonitoredSession(
          session_creator=monitored_session.ChiefSessionCreator(
              checkpoint_filename_with_path=checkpoint_path,
              scaffold=infer_ops.scaffold,
              config=self._session_config))
    tf.logging.info('get session done')

  def my_infer_model(self, input_fn, feed_fn=None, outputs=None,
                     as_iterable=True, iterate_batches=False):
     # self.features = self._get_features_from_input_fn(input_fn)
     # self.infer_ops = self._get_predict_ops(self.features)
     # self.predictions = self._filter_predictions(self.infer_ops.predictions, outputs)
     predictions = self.predictions
     with ops.Graph().as_default() as g:
        mon_sess = self.mon_sess
        if not as_iterable:
           with mon_sess:
             if not mon_sess.should_stop():
               return mon_sess.run(predictions,feed_fn)# feed_fn() if feed_fn else None)
        else:
  ...
  def predict(self, x=None, input_fn=None, feed_fn=None, batch_size=None, outputs=None, as_iterable=True):
    ...
    """
    _verify_input_args(x, None, input_fn, feed_fn, batch_size)
    if x is not None and not as_iterable:
      return SKCompat(self).predict(x, batch_size)

    input_fn, feed_fn = _get_input_fn(x, None, input_fn, feed_fn, batch_size)
    return self.my_infer_model(
        input_fn=input_fn,
        feed_fn=feed_fn,
        outputs=outputs,
  ...
  def _predict_generator(self, mon_sess, predictions, feed_fn, iterate_batches):
    ...
    if feed_fn:
       ...
       feed = {self.inputs_ph: inputs}
    #while not mon_sess.should_stop():
    for i in range(1):
        preds = mon_sess.run(predictions, feed)#feed_fn() if feed_fn else None)
        #print('type of preds is:', type(preds), preds.keys())
        #print(len(preds[preds.keys()[0]]),len(preds[preds.keys()[1]]))
        if iterate_batches:
          yield preds
...
class Estimator(BaseEstimator):
  ...
  def _get_eval_ops(self, features, labels, metrics):
    ...
    if metrics:
      # if 'metrics-wmt_ende_bpe32k/oov_trans_failed' in metrics.keys() or 'metrics/oov_trans_failed' in metrics.keys():
      #     #tf.logging.info("oov in metrics\n")
      #     problem_id = 0
      #     targets_vocab = self.hparams.problems[problem_id].vocabulary["targets"]
      #     dict2num=[targets_vocab.encode(line)[0] for line in open('oov.en','r',encoding='utf-8')]
      # else:
      #     dict2num=None
      #tf.logging.info('dict2num%s',dict2num)
