...
class Transformer(t2t_model.T2TModel):
  ...
  tf.logging.info('use this Transformer')
...
def transformer_prepare_encoder(inputs, target_space, hparams):
  ...
  emb_target_space = common_layers.embedding(
      target_space, 32, ishape_static[-1], name="target_space_embedding",reuse=tf.AUTO_REUSE)
...
@registry.register_hparams
def transformer_paraphrase_small():
  """HParams for transfomer small model on paraphrase."""
  hparams = transformer_base()
  #hparams.hidden_size = 512
...
def transformer_big_single_gpu():
  ...
  tf.logging.info('use this transformer_big_single_gpu')
...
@registry.register_hparams
def transformer_big2_single_gpu():
  """HParams for transformer big model for single gpu."""
  tf.logging.info('use this transformer_big2_single_gpu')
  hparams = transformer_big()
  hparams.hidden_size = 960
  hparams.num_hidden_layers = 8
  hparams.num_heads = 24
  hparams.residual_dropout = 0.1
  hparams.learning_rate_warmup_steps = 16000
  hparams.optimizer_adam_beta2 = 0.998
  hparams.batching_mantissa_bits = 3
...
@registry.register_hparams
def transformer_big3_single_gpu():
  """HParams for transformer big model for single gpu."""
  tf.logging.info('use this transformer_big3_single_gpu')
  hparams = transformer_big()
  hparams.hidden_size = 1024
  hparams.num_hidden_layers = 8
